...
describe("greaterThan10", function() {
  ...
  it("should return the correct value", function() {
    expect(
      greaterThan10({ one: 1, two: 2, three: 11, four: 44, five: 5 })
    ).toEqual({ one: 1, two: 2, three: 0, four: 0, five: 5 });
    expect(
      greaterThan10({ one: 10, two: 20, three: 11, four: 4, five: 335 })
    ).toEqual({ one: 10, two: 0, three: 0, four: 4, five: 0 });
  });
});
...
describe("double", function() {
  ...
  it("should return the correct value", function() {
    expect(double({ one: 1, two: 2, three: 3 })).toEqual({
      one: 2,
      two: 4,
      three: 6
    });
...
describe("secrets", function() {
  ...
  it("should return the correct value", function() {
    ...
    ).toBe("I really, really love javascript!");

    expect(
      secrets({
        sh: "Cheese",
        sally: "this is no secret",
        shake: " is",
        shed: " disgusting :(",
        sue: "this also does not belong",
        s: "nope!"
      })
    ).toBe("Cheese is disgusting :(");
  });
});
...
describe("removePassword", function() {
  ...
  it("should return the correct value", function() {
    expect(
      removePassword({ username: "happyCow23", password: "123jumptheMoon" })
    ).toEqual({ username: "happyCow23" });
    expect(
      removePassword({ username: "JSQueen", password: "es6RulesTheWorld" })
    ).toEqual({ username: "JSQueen" });
  });
});
...
describe("startsWithK", function() {
  ...
  it("should return the correct value", function() {
    expect(startsWithK({ kitty: 2, dog: 3, cat: 4 })).toEqual({
      dog: 3,
      cat: 4
    });
    expect(startsWithK({ snake: 2, cougar: 3, kangaroo: 4 })).toEqual({
      snake: 2,
      cougar: 3
    });
  });
...
describe("hiddenTreasure", function() {
  ...
  it("should return the correct value", function() {
    ...
    ).toEqual({ two: "puppies are always a treasure" });
  });
});
